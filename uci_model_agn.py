...
def main(from_main_py=False, args=None):
    ...
    if from_main_py:
        # When called from main.py, we only process one dataset/seed
        # Map main.py arguments to uci_model_agn arguments
        dataset = args.data  # main.py uses 'data' instead of 'dataset'
        seed = args.seed
        datasets = [dataset]
        SEEDS = [seed]
        
        # Map other arguments
        args.n_hidden_layers = args.nl
        args.n_hidden_units = args.hs
        args.n_epochs = args.num_ep
        args.overfit_length = args.wait
        args.gpu = str(args.gpu)
        args.debug = int(args.debug)
        args.recal = int(args.recal)
        args.big_model = 0
        args.noisy = 0
    else:
        # Original behavior when run standalone
        if args.dataset == '':
            if args.noisy:
                #datasets = ['yacht_normal_0.1','yacht_normal_1','yacht_normal_10','yacht_uniform_0.1','yacht_uniform_1','yacht_uniform_10','boston_normal_0.1','boston_normal_1','boston_normal_10','boston_uniform_0.1','boston_uniform_1','boston_uniform_10','energy_normal_0.1','energy_normal_1','energy_normal_10','energy_uniform_0.1','energy_uniform_1','energy_uniform_10','concrete_normal_0.1','concrete_normal_1','concrete_normal_10','concrete_uniform_0.1','concrete_uniform_1','concrete_uniform_10','wine_normal_0.1','wine_normal_1','wine_normal_10','wine_uniform_0.1','wine_uniform_1','wine_uniform_10','power_normal_0.1',
                datasets = ['power_normal_1','power_normal_10','power_uniform_0.1','power_uniform_1','power_uniform_10','kin8nm_normal_0.1','kin8nm_normal_1','kin8nm_normal_10','kin8nm_uniform_0.1','kin8nm_uniform_1','kin8nm_uniform_10','naval_normal_0.1','naval_normal_1','naval_normal_10','naval_uniform_0.1','naval_uniform_1','naval_uniform_10','protein_normal_0.1','protein_normal_1','protein_normal_10','protein_uniform_0.1','protein_uniform_1','protein_uniform_10']
            else:
    ...
    for dataset in datasets:
        ...
        for s in SEEDS:
            ...
            if args.big_model:
                f_path = f'maqr/big_mean_models/{dataset}_{seed}_mean.pkl'
            else:
                f_path = f'maqr/mean_models/{dataset}_{seed}_mean.pkl'
                
            if not os.path.exists(f_path):
                # if args.big_model:
                #     gen_model(dataset, seed, f_path, device, base_model = standard_nn_model)
                # else:
                if hasattr(args, 'mean_model') and args.mean_model:
                    gen_model(dataset, seed, f_path, device, base_model=args.mean_model, base_model_args=args.mean_model_args, train_val_data=(x_tr, x_va, y_tr, y_va))
                else:
            ...
            if from_main_py:
                cdf_model = QModelEns(input_size=cdf_x_tr_tensor.shape[1],
                                    ...
                                    num_ens=args.num_ens, device=device,
                                    residual=args.residual,
                                    batch_norm=args.batch_norm,
                                    layer_norm=args.layer_norm,
                                    dropout=args.dropout,
                                    activation=args.activation)
                # If called from main.py, return the trained model and data
                return cdf_model, loader_cdf, cdf_x_va_tensor, cdf_y_va_tensor, pred_va, pred_te
            else:
            ...
            cdf_best_model = None
            va_loss = []
            for epoch in tqdm.tqdm(range(args.num_ep if from_main_py else num_epoch)):
                ...
                ep_va_loss = cdf_model.update_va_loss(mse_loss_fn, x_va, y_va, q_list=None, batch_q=True, curr_ep=epoch, num_wait=args.wait if from_main_py else args.overfit_length, args=args)
        ...
        label = f'mauq_recal_{args.recal}_bigmodel_{args.big_model}_noisy_{args.noisy}'
